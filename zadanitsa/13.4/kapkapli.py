...
class KapKap:
    ...
    def __init__(self):
        ...
        self.kapkap = pygame.sprite.Group()
        self._create_drops()
    ...
    def _create_drop(self, drop_number, row_number):
        ...
        drop.rect.y = drop.y
        self.kapkap.add(drop)

    def _update_kapkap(self):
        self.kapkap.update()

        make_new_drops = False
        for drop in self.kapkap.copy():
            if drop.check_disappeared():
                self.kapkap.remove(drop)
                make_new_drops = True
